...
def run(sc, input_file, output_dir, cateory='job_ads'):
    ...
    # for file_/Users/ZwEin/job_works/StudentWork_USC-ISI/projects/wedc-one-class-classification/wedc/spark_loader.pypath in os.listdir(files_dir):
    #     if file_path[0] != '.':
    ...
    rdd_content = rdd_original.map(map_load_data(keywords))
    rdd_content = rdd_content.values()

    def my_method(line):
        return [json.dumps(line)] if not isinstance(line, basestring) else []
